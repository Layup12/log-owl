...
Desktop‑приложение для трекинга времени и задач, построенное на Electron, React и TypeScript. Это pet‑project, в котором я **показываю продакшен‑подходы** (типобезопасный IPC, строгий линтинг, миграции БД, автотесты и e2e, сборка через CI), но без претензии на идеальный продукт.
...
- **Версия:** 0.1.0 (early preview, pet‑project в активной разработке).
- **Стабильный функционал:** базовый трекинг времени, локальное хранение в SQLite, типобезопасный IPC, сборка desktop‑приложения.
- **Развитие:** активная доработка UX и фич; подробнее — в `docs/BACKLOG.md`.
- **Качество:** строгий линтинг, юнит‑ и e2e‑тесты, покрытие кода и проверки в CI на каждый push.
...
- **Качество кода и проверки**
  ...
  - **Vitest** — юнит‑тесты; в проекте сделан **упор на тестирование** критичной и важной логики (интервалы времени, парсинг, валидация контрактов).
  - **Vitest Coverage (провайдер v8)** — отслеживание покрытия кода; порог пока низкий, цель — повышать по мере добавления тестов.
  - **Playwright** — e2e‑тесты (сценарии в `tests/e2e/`, конфиг `playwright.config.ts`).
...
- `pnpm run test:coverage` — юнит/компонентные тесты и проверка порога покрытия (Vitest);
- `pnpm run e2e` — e2e‑тесты (Playwright).

Хук pre-commit через Husky запускает `lint-staged` и `pnpm run test:coverage`, поэтому большинство проблем и регрессий покрытия ловится до пуша.
...
Историю изменений по версиям см. в [`CHANGELOG.md`](CHANGELOG.md).
...
- **`tests/`** — инфраструктура тестов: `vitest.setup.ts`, e2e‑сценарии Playwright в `tests/e2e/`.
...
- При этом я сознательно вкладываю сюда практики, которые считаю полезными в продакшене:
  ...
  - автотесты и отслеживание покрытия (порог пока низкий, цель — повышать со временем);
