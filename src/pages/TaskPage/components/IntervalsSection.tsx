...
import { formatUtcLocal } from '@shared/lib'
...
export function IntervalsList({ state }: IntervalsSectionProps) {
  return (
    <>
              ...
              state.editingEntryId === entry.id ? (
                <ListItem key={entry.id} sx={{ flexWrap: 'wrap', gap: 1 }}>
                  ...
                  <TextField
                    ...
                    sx={{ minWidth: 200 }}
                  />
                  <IconButton size="small" onClick={state.saveEditEntry} aria-label="Сохранить">
                    <SaveIcon />
                  </IconButton>
                  <IconButton size="small" onClick={state.cancelEditEntry} aria-label="Отмена">
                    <CloseIcon />
                  </IconButton>
                </ListItem>
              ) : (
                <ListItem key={entry.id}>
                  ...
                  />
                  <ListItemSecondaryAction>
                    <IconButton size="small" onClick={() => state.startEditEntry(entry)} aria-label="Редактировать">
                      <EditIcon />
                    </IconButton>
                    <IconButton size="small" onClick={() => state.openDeleteEntryDialog(entry)} aria-label="Удалить">
                      <DeleteIcon />
                    </IconButton>
                  </ListItemSecondaryAction>
                </ListItem>
      ...
      <Dialog open={state.deleteConfirmEntry !== null} onClose={state.closeDeleteEntryDialog}>
        <DialogTitle>Удалить интервал?</DialogTitle>
        <DialogContent>
          {state.deleteConfirmEntry && (
            <Typography>
              {formatUtcLocal(state.deleteConfirmEntry.started_at)} —{' '}
              {state.deleteConfirmEntry.ended_at ? formatUtcLocal(state.deleteConfirmEntry.ended_at) : '…'}
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={state.closeDeleteEntryDialog}>Отмена</Button>
          <Button color="error" variant="contained" onClick={state.handleDeleteEntry}>
            Удалить
          </Button>
        </DialogActions>
      </Dialog>
